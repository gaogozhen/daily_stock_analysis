...
| `STOCK_LIST_FETCH_API` | Optional HTTP(S) endpoint for a dynamic watchlist; supports plain text, JSON arrays, or JSON objects with `stocks` / `stock_list` / `codes`; the request does not reuse `HTTP_PROXY`/`HTTPS_PROXY`, while loopback, link-local, and cloud metadata addresses are blocked; process-level env set before startup has highest priority over WebUI-written `.env` values | - |

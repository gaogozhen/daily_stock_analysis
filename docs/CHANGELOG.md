...
- [修复] 异步 Extension Action 完成后通过任务状态、任务列表和 SSE 完成事件暴露 `ActionResult`，避免轮询客户端只能看到 completed 而取不到插件输出。
...
- [文档] 明确本轮 Extension Runtime MVP 的外部模型/API 兼容核验边界：未改动 `src/config.py` 与 `.env` 迁移链路、未新增 provider/model/base URL 的运行时语义；核验证据为 `tests/test_extensions_runtime.py` 与 `tests/test_task_queue_config_sync.py`，回退路径为版本回退，不需要配置补偿步骤。
